...
class Attention(nn.Module):
    ...
    def forward(self, encoder_out, decoder_hidden):
        ...
        a = self.softmax(att)
        attention_weighted_encoding = (encoder_out * a.unsqueeze(2)).sum(dim=1)
...
class DecoderWithAttention(nn.Module):
    ...
    def forward(self, word, decoder_hidden_state, decoder_cell_state, encoder_out):
        ...
        decoder_hidden_state, decoder_cell_state = self.decode_step(torch.cat((embedded, self.attention(encoder_out, decoder_hidden_state)), 1), (decoder_hidden_state, decoder_cell_state))
